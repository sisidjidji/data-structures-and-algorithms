...
const divisibleByFiveTwoToThePower = (input) => {
  let arr=[];
   ...
   let y= element.filter(num=> (typeof num ==='number' && num%5===0)).map(num=> Math.pow(2,num));
   arr.push(y)
  });
return arr;
};
...
let findMaleAndFemale = (data) => {
 return data.filter((obj)=>(obj.gender==='male' || obj.gender==='female')).map(x=> x.name).join(' and ');
